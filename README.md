...
Telegram-бот для организации совместных поездок и доставки продуктов.

## 📋 Описание

**Ride Share Bot** — это умный помощник, который соединяет водителей и пассажиров, путешествующих по одному маршруту, а также помогает заказывать доставку продуктов из магазинов. Бот использует искусственный интеллект для распознавания естественного языка и автоматического создания заявок.
...
- 🛍 **Доставка** — заказ продуктов из магазинов («Привези» / «Привезу»)
...
| `/my_deliveries` | Мои доставки (заявки и предложения) |
...
- **📦 Мои доставки** — управление заказами
...
**Для доставки (заказ):**
```
Пользователь: 🛍 Привези
Бот: Напишите, что нужно привезти. Например:
     "Привези продукты из Пятёрочки сегодня до 18:00"

Пользователь: Привези молоко, хлеб и яйца из Пятёрочки сегодня в 18:00
Бот: 🛍 Подтверждение заявки
     🏪 Магазин: Пятёрочка
     📅 Дата: 15.03.2024
     🕐 Время: 18:00
...
Бот: ✅ Заявка на доставку сохранена!
```

**Для доставки (предложение):**
```
Пользователь: 🛒 Привезу
Бот: Напишите, куда вы едете. Например:
     "Еду в Пятёрочку сегодня в 15:00, могу взять 3 заказа"

Пользователь: Еду в Пятёрочку сегодня в 15:00, могу взять 3 заказа
Бот: ✅ Предложение доставки сохранено!
```
