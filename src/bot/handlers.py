...
def main_kb():
    kb = [
        ...
        [KeyboardButton(text="🔍 Найти попутчиков"), KeyboardButton(text="📋 Мои поездки")]
    ]
...
async def handle_ai_conversation(m: types.Message, state: FSMContext):
    ...
    # Если ИИ собрал полные данные (JSON) — сохраняем поездку
    if res.get("origin") and res.get("destination") and res.get("date"):
        await process_ride_data(m, res, state)
    
    # Выводим текстовый ответ от ИИ (болтовня, уточнение или подтверждение)
...
async def process_ride_data(m: types.Message, res: dict, state: FSMContext):
    ...
    async with async_session() as s:
        ...
        seats = int(res.get('seats', 1 if role == 'passenger' else 3))  # Default для driver 3, если не указано
        new_ride = Ride(
            user_id=user.id,
            origin=res['origin'],
            destination=res['destination'],
            ride_date=res['date'],  # Предполагаем нормализованный формат от NLU, e.g. "2025-12-27"
        ...
        # Автоматический поиск совпадений только для водителей
        if role == 'driver':
    ...
    async with async_session() as s:
        ...
        for r_obj, match_user in matches:
            if new_ride.seats > 0:
                ...
                match_msg = (
                    f"🔔 *Найден попутчик!*\n"
                    f"📍 {r_obj.origin} ➡️ {r_obj.destination}\n"
                    f"📅 Дата: {r_obj.ride_date}\n"
                    f"🕒 Время: {r_obj.start_time}\n"
                    f"👤 Контакт: @{match_user.username or 'скрыт'}"
                )
                try:
